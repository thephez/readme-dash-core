...
| ---- | ----------- | --------- | ----------- |
| Net | GetBlockTxn Index Error | **100** | Peer relayed a [`getblocktxn` message](core-ref-p2p-network-data-messages.md#sectiongetblocktxn) with out-of-bound indices
| Net | Bloom Filter Service | **100** | Bloom filter message received from peer that has bloom filter commands disabled by default (protocol version > 70201) (`filterload` message, [`filteradd` message](core-ref-p2p-network-control-messages.md#sectionfilteradd), or [`filterclear` message](core-ref-p2p-network-control-messages.md#sectionfilterclear))
| Net | Block Rejected | 1 | Peer rejected the block it requested from us
| Net | Duplicate Version | 1 | Duplicate [`version` message](core-ref-p2p-network-control-messages.md#sectionversion) received
...
| Net | No Version | 1 | Received a message prior to receiving a [`version` message](core-ref-p2p-network-control-messages.md#sectionversion)
| Net | No Verack | 1 | After sending [`version` message](core-ref-p2p-network-control-messages.md#sectionversion), received a message other than a [`verack` message](core-ref-p2p-network-control-messages.md#sectionverack) back first
...
| Net | Bloom Filter Size | **100** | Maximum script element size (520) exceeded (`filterload` message or [`filteradd` message](core-ref-p2p-network-control-messages.md#sectionfilteradd))
...
| LLMQ Signing | Too Many Messages | **100** | Maximum message count exceed in [`qsigsesann` message](core-ref-p2p-network-quorum-messages.md#sectionqsigsesann), [`qsigsinv` message](core-ref-p2p-network-quorum-messages.md#sectionqsigsinv), [`qgetsigs` message](core-ref-p2p-network-quorum-messages.md#sectionqgetsigs), or [`qbsigs` message](core-ref-p2p-network-quorum-messages.md#sectionqbsigs)
...
| Governance | Unsupported Vote Signal | 20 | Vote signal outside the accepted range (see [`govobjvote` message](core-ref-p2p-network-governance-messages.md#sectiongovobjvote))
