...
Since the messages are all related to a single process, this diagram shows them sequentially numbered. The [`dssu` message](core-ref-p2p-network-privatesend-messages.md#sectiondssu) (not shown) is sent by the masternode in conjunction with some responses. For additional details, refer to the Developer Guide [PrivateSend section](core-guide-dash-features-privatesend).
...
The [`dsa` message](core-ref-p2p-network-privatesend-messages.md#sectiondsa) allows a <<glossary:node>> to join a mixing pool. A collateral fee is required and may be forfeited if the client acts maliciously. The message operates in two ways:
...
| 216+ | txCollateral | [`tx` message](core-ref-p2p-network-data-messages.md#sectiontx) | Required | Collateral TX that will be charged if this client acts maliciously

The following annotated hexdump shows a [`dsa` message](core-ref-p2p-network-privatesend-messages.md#sectiondsa). (The message header has been omitted.) Note that the 'Required inputs' bytes will only be preset if <<glossary:Spork>> 6 is active and protocol version => 70209.
...
The [`dsc` message](core-ref-p2p-network-privatesend-messages.md#sectiondsc) indicates a PrivateSend mixing session is complete.
...
Reference the Message IDs table under the [`dssu` message](core-ref-p2p-network-privatesend-messages.md#sectiondssu) for descriptions of the Message ID values.

The following annotated hexdump shows a [`dsc` message](core-ref-p2p-network-privatesend-messages.md#sectiondsc). (The message header has been omitted.)
...
The [`dsf` message](core-ref-p2p-network-privatesend-messages.md#sectiondsf) is sent by the masternode as the final mixing transaction in a PrivateSend mixing session. The <<glossary:masternode>> expects <<glossary:nodes>> in the mixing session to respond with a [`dss` message](core-ref-p2p-network-privatesend-messages.md#sectiondss).

| Bytes | Name | Data type | Required | Description |
| ---------- | ----------- | --------- | -------- | -------- |
| 4 | nSessionID | int | Required | ID of the mixing session
| # | txFinal | [`tx` message](core-ref-p2p-network-data-messages.md#sectiontx) | Required |  Final mixing transaction with unsigned inputs

The following annotated hexdump shows a [`dsf` message](core-ref-p2p-network-privatesend-messages.md#sectiondsf). (The message header has been omitted.) Transaction <<glossary:inputs>>/<<glossary:outputs>> are only shown for a single node (compare with the [`dsi` message](core-ref-p2p-network-privatesend-messages.md#sectiondsi) and [`dss` message](core-ref-p2p-network-privatesend-messages.md#sectiondss) hexdumps).
...
The [`dsi` message](core-ref-p2p-network-privatesend-messages.md#sectiondsi) replies to a [`dsq` message](core-ref-p2p-network-privatesend-messages.md#sectiondsq) that has the Ready field set to 0x01. The [`dsi` message](core-ref-p2p-network-privatesend-messages.md#sectiondsi) contains user <<glossary:inputs>> for mixing along with the <<glossary:outputs>> and a collateral. Once the <<glossary:masternode>> receives [`dsi` messages](core-ref-p2p-network-privatesend-messages.md#sectiondsi) from all members of the pool, it responds with a [`dsf` message](core-ref-p2p-network-privatesend-messages.md#sectiondsf).

| Bytes | Name | Data type | Required | Description |
| ---------- | ----------- | --------- | -------- | -------- |
| ? | vecTxDSIn | CTxDSIn[] | Required | Vector of users inputs (CTxDSIn serialization is equal to CTxIn serialization)
| 216+ | txCollateral | [`tx` message](core-ref-p2p-network-data-messages.md#sectiontx) | Required | Collateral transaction which is used to prevent misbehavior and also to charge fees randomly
| ? | vecTxDSOut | CTxDSOut[] | Required | Vector of user outputs (CTxDSOut serialization is equal to CTxOut serialization)

The following annotated hexdump shows a [`dsi` message](core-ref-p2p-network-privatesend-messages.md#sectiondsi). (The message header has been omitted.)
...
The [`dsq` message](core-ref-p2p-network-privatesend-messages.md#sectiondsq) provides <<glossary:nodes>> with mixing queue details and notifies them when to sign final mixing TX messages.

If the message indicates the queue is not ready, the node verifies the message is valid. It also verifies that the <<glossary:masternode>> is not flooding the <<glossary:network>> with [`dsq` messages](core-ref-p2p-network-privatesend-messages.md#sectiondsq) in an attempt to dominate the queuing process. It then relays the message to its connected <<glossary:peers>>.

If the message indicates the queue is ready, the node responds with a [`dsi` message](core-ref-p2p-network-privatesend-messages.md#sectiondsi).
...
| 8 | nTime | int64_t | Required | Time this [`dsq` message](core-ref-p2p-network-privatesend-messages.md#sectiondsq) was created
...
The following annotated hexdump shows a [`dsq` message](core-ref-p2p-network-privatesend-messages.md#sectiondsq). (The message header has been omitted.) Note that the 'Required <<glossary:inputs>>' bytes will only be preset if <<glossary:Spork>> 6 is active and protocol version => 70209.
...
The [`dss` message](core-ref-p2p-network-privatesend-messages.md#sectiondss) replies to a [`dsf` message](core-ref-p2p-network-privatesend-messages.md#sectiondsf) sent by the <<glossary:masternode>> managing the mixing session.  The [`dsf` message](core-ref-p2p-network-privatesend-messages.md#sectiondsf) provides the unsigned transaction <<glossary:inputs>> for all members of the mixing pool. Each <<glossary:node>> verifies that the final transaction matches what is expected. They then sign any transaction inputs belonging to them and then relay them to the masternode via this [`dss` message](core-ref-p2p-network-privatesend-messages.md#sectiondss).

Once the masternode receives and validates all [`dss` messages](core-ref-p2p-network-privatesend-messages.md#sectiondss), it issues a [`dsc` message](core-ref-p2p-network-privatesend-messages.md#sectiondsc). If a node does not respond to a [`dsf` message](core-ref-p2p-network-privatesend-messages.md#sectiondsf) with signed transaction inputs, it may forfeit the collateral it provided. This is to minimize malicious behavior.

| Bytes | Name | Data type | Required | Description |
| ---------- | ----------- | --------- | -------- | -------- |
| # | inputs | txIn[] | Required | Signed inputs for mixing session

The following annotated hexdump shows a [`dss` message](core-ref-p2p-network-privatesend-messages.md#sectiondss). (The message header has been omitted.) Note that these will be the same transaction inputs that were supplied (unsiged) in the [`dsi` message](core-ref-p2p-network-privatesend-messages.md#sectiondsi).
...
The [`dssu` message](core-ref-p2p-network-privatesend-messages.md#sectiondssu) provides a mixing pool status update.
...
The following annotated hexdump shows a [`dssu` message](core-ref-p2p-network-privatesend-messages.md#sectiondssu). (The message header has been omitted.)
...
The [`dstx` message](core-ref-p2p-network-privatesend-messages.md#sectiondstx) allows <<glossary:masternodes>> to broadcast subsidized transactions without fees (to provide security in mixing).

| Bytes | Name | Data type | Required | Description |
| ---------- | ----------- | --------- | -------- | -------- |
| # | tx | [`tx` message](core-ref-p2p-network-data-messages.md#sectiontx) | Required | The transaction
...
The following annotated hexdump shows a [`dstx` message](core-ref-p2p-network-privatesend-messages.md#sectiondstx). (The message header has been omitted.)
