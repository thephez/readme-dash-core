...
The `addr` (IP address) message relays connection information for peers on the network. Each peer which wants to accept incoming connections creates an [`addr` message](core-ref-p2p-network-control-messages.md#sectionaddr) providing its connection information and then sends that message to its peers unsolicited. Some of its peers send that information to their peers (also unsolicited), some of which further distribute it, allowing decentralized peer discovery for any program already on the network.

An [`addr` message](core-ref-p2p-network-control-messages.md#sectionaddr) may also be sent in response to a [`getaddr` message](core-ref-p2p-network-control-messages.md#sectiongetaddr).
...
| Bytes | Name       | Data Type | Description
|-------|------------|-----------|---------------
| 4     | time       | uint32    | *Added in protocol version 31402.* <br><br>A time in Unix epoch time format.  Nodes advertising their own IP address set this to the current time.  Nodes advertising IP addresses they've connected to set this to the last time they connected to that node.  Other nodes just relaying the IP address should not change the time.  Nodes can use the time field to avoid relaying old [`addr` messages](core-ref-p2p-network-control-messages.md#sectionaddr).  <br><br>Malicious nodes may change times or even set them in the future.
| 8     | services   | uint64_t  | The services the node advertised in its [`version` message](core-ref-p2p-network-control-messages.md#sectionversion).
...
The following annotated hexdump shows part of an [`addr` message](core-ref-p2p-network-control-messages.md#sectionaddr). (The <<glossary:message header>> has been omitted and the actual IP address has been replaced with a [RFC5737](http://tools.ietf.org/html/rfc5737) reserved IP address.)
...
The [`filteradd` message](core-ref-p2p-network-control-messages.md#sectionfilteradd) tells the receiving <<glossary:peer>> to add a single element to a previously-set <<glossary:bloom filter>>, such as a new <<glossary:public key>>. The element is sent directly to the receiving peer; the peer then uses the parameters set in the [`filterload` message](core-ref-p2p-network-control-messages.md#sectionfilterload) to add the element to the bloom filter.

Because the element is sent directly to the receiving peer, there is no obfuscation of the element and none of the plausible-deniability privacy provided by the bloom filter. Clients that want to maintain greater privacy should recalculate the bloom filter themselves and send a new [`filterload` message](core-ref-p2p-network-control-messages.md#sectionfilterload) with the recalculated bloom filter.
...
Note: a [`filteradd` message](core-ref-p2p-network-control-messages.md#sectionfilteradd) will not be accepted unless a filter was previously set with the [`filterload` message](core-ref-p2p-network-control-messages.md#sectionfilterload).

The annotated hexdump below shows a [`filteradd` message](core-ref-p2p-network-control-messages.md#sectionfilteradd) adding a <<glossary:TXID>>. (The message header has been omitted.) This TXID appears in the same block used for the example hexdump in the [`merkleblock` message](core-ref-p2p-network-data-messages.md#sectionmerkleblock); if that [`merkleblock` message](core-ref-p2p-network-data-messages.md#sectionmerkleblock) is re-sent after sending this [`filteradd` message](core-ref-p2p-network-control-messages.md#sectionfilteradd), six hashes are returned instead of four.
...
The [`filterclear` message](core-ref-p2p-network-control-messages.md#sectionfilterclear) tells the receiving <<glossary:peer>> to remove a previously-set <<glossary:bloom filter>>.  This also undoes the effect of setting the relay field in the [`version` message](core-ref-p2p-network-control-messages.md#sectionversion) to 0, allowing unfiltered access to [`inv` messages](core-ref-p2p-network-data-messages.md#sectioninv) announcing new transactions.

Dash Core does not require a [`filterclear` message](core-ref-p2p-network-control-messages.md#sectionfilterclear) before a replacement filter is loaded with `filterload`.  It also doesn't require a [`filterload` message](core-ref-p2p-network-control-messages.md#sectionfilterload) before a [`filterclear` message](core-ref-p2p-network-control-messages.md#sectionfilterclear).

There is no payload in a [`filterclear` message](core-ref-p2p-network-control-messages.md#sectionfilterclear).  See the [message header section](core-ref-p2p-network-message-headers) for an example of a message without a payload.

# filterload

*Added in protocol version 70001 as described by BIP37.*

The [`filterload` message](core-ref-p2p-network-control-messages.md#sectionfilterload) tells the receiving <<glossary:peer>> to filter all relayed transactions and requested <<glossary:merkle blocks>> through the provided filter. This allows clients to receive transactions relevant to their <<glossary:wallet>> plus a configurable rate of false positive transactions which can provide plausible-deniability privacy.
...
The annotated hexdump below shows a [`filterload` message](core-ref-p2p-network-control-messages.md#sectionfilterload). (The message header has been omitted.)  For an example of how this payload was created, see the [filterload example](core-examples-p2p-network-creating-a-bloom-filter).
...
The [`getaddr` message](core-ref-p2p-network-control-messages.md#sectiongetaddr) requests an [`addr` message](core-ref-p2p-network-control-messages.md#sectionaddr) from the receiving <<glossary:node>>, preferably one with lots of IP addresses of other receiving nodes. The transmitting node can use those IP addresses to quickly update its database of available nodes rather than waiting for unsolicited [`addr` messages](core-ref-p2p-network-control-messages.md#sectionaddr) to arrive over time.

There is no payload in a [`getaddr` message](core-ref-p2p-network-control-messages.md#sectiongetaddr).  See the [message header section](core-ref-p2p-network-message-headers) for an example of a message without a payload.

# getsporks

The [`getsporks` message](core-ref-p2p-network-control-messages.md#sectiongetsporks) requests [`spork` messages](core-ref-p2p-network-control-messages.md#sectionspork) from the receiving node.

There is no payload in a [`getsporks` message](core-ref-p2p-network-control-messages.md#sectiongetsporks).  See the [message header section](core-ref-p2p-network-message-headers) for an example of a message without a payload.

# ping

The [`ping` message](core-ref-p2p-network-control-messages.md#sectionping) helps confirm that the receiving <<glossary:peer>> is still connected. If a TCP/IP error is encountered when sending the [`ping` message](core-ref-p2p-network-control-messages.md#sectionping) (such as a connection timeout), the transmitting node can assume that the receiving node is disconnected. The response to a [`ping` message](core-ref-p2p-network-control-messages.md#sectionping) is the [`pong` message](core-ref-p2p-network-control-messages.md#sectionpong).

Before protocol version 60000, the [`ping` message](core-ref-p2p-network-control-messages.md#sectionping) had no payload. As of protocol version 60001 and all later versions, the message includes a single field, the nonce.

| Bytes | Name  | Data Type | Description
|-------|-------|-----------|---------------
| 8     | nonce | uint64_t  | *Added in protocol version 60001 as described by BIP31.* <br><br>Random nonce assigned to this [`ping` message](core-ref-p2p-network-control-messages.md#sectionping).  The responding [`pong` message](core-ref-p2p-network-control-messages.md#sectionpong) will include this nonce to identify the [`ping` message](core-ref-p2p-network-control-messages.md#sectionping) to which it is replying.

The annotated hexdump below shows a [`ping` message](core-ref-p2p-network-control-messages.md#sectionping). (The message header has been omitted.)
...
The [`pong` message](core-ref-p2p-network-control-messages.md#sectionpong) replies to a [`ping` message](core-ref-p2p-network-control-messages.md#sectionping), proving to the pinging <<glossary:node>> that the ponging node is still alive. Dash Core will, by default, disconnect from any clients which have not responded to a [`ping` message](core-ref-p2p-network-control-messages.md#sectionping) within 20 minutes.

To allow nodes to keep track of latency, the [`pong` message](core-ref-p2p-network-control-messages.md#sectionpong) sends back the same nonce received in the [`ping` message](core-ref-p2p-network-control-messages.md#sectionping) it is replying to.

The format of the [`pong` message](core-ref-p2p-network-control-messages.md#sectionpong) is identical to the [`ping` message](core-ref-p2p-network-control-messages.md#sectionping); only the message header differs.

# reject

*Added in protocol version 70002 as described by BIP61.*

The [`reject` message](core-ref-p2p-network-control-messages.md#sectionreject) informs the receiving <<glossary:node>> that one of its previous messages has been rejected.
...
|------|-------------------|-------------|------------|----------------
| 0x01 | *any message*     | 0           | N/A        | Message could not be decoded.  Be careful of [`reject` message](core-ref-p2p-network-control-messages.md#sectionreject) feedback loops where two peers each don't understand each other's [`reject` messages](core-ref-p2p-network-control-messages.md#sectionreject) and so keep sending them back and forth forever.
| 0x10 | [`block` message](core-ref-p2p-network-data-messages.md#sectionblock)   | 32          | char[32]   | Block is invalid for some reason (invalid proof-of-work, invalid signature, etc).  Extra data may include the rejected block's header hash.
| 0x10 | [`tx` message](core-ref-p2p-network-data-messages.md#sectiontx)      | 32          | char[32]   | Transaction is invalid for some reason (invalid signature, output value greater than input, etc.).  Extra data may include the rejected transaction's TXID.
...
| 0x11 | [`block` message](core-ref-p2p-network-data-messages.md#sectionblock)   | 32          | char[32]   | The block uses a version that is no longer supported.  Extra data may include the rejected block's header hash.
| 0x11 | [`version` message](core-ref-p2p-network-control-messages.md#sectionversion) | 0           | N/A        | Connecting node is using a protocol version that the rejecting node considers obsolete and unsupported.
| 0x11 | [`dsa` message](core-ref-p2p-network-privatesend-messages.md#sectiondsa)     | 0           | N/A        | Connecting node is using a PrivateSend protocol version that the rejecting node considers obsolete and unsupported.
| 0x11 | [`dsi` message](core-ref-p2p-network-privatesend-messages.md#sectiondsi)     | 0           | N/A        | Connecting node is using a PrivateSend protocol version that the rejecting node considers obsolete and unsupported.
| 0x11 | [`dsc` message](core-ref-p2p-network-privatesend-messages.md#sectiondsc)     | 0           | N/A        | Connecting node is using a PrivateSend protocol version that the rejecting node considers obsolete and unsupported.
| 0x11 | [`dsf` message](core-ref-p2p-network-privatesend-messages.md#sectiondsf)     | 0           | N/A        | Connecting node is using a PrivateSend protocol version that the rejecting node considers obsolete and unsupported.
| 0x11 | [`dsq` message](core-ref-p2p-network-privatesend-messages.md#sectiondsq)     | 0           | N/A        | Connecting node is using a PrivateSend protocol version that the rejecting node considers obsolete and unsupported.
| 0x11 | [`dssu` message](core-ref-p2p-network-privatesend-messages.md#sectiondssu)    | 0           | N/A        | Connecting node is using a PrivateSend protocol version that the rejecting node considers obsolete and unsupported.
| 0x11 | [`govsync` message](core-ref-p2p-network-governance-messages.md#sectiongovsync) | 0           | N/A        | Connecting node is using a governance protocol version that the rejecting node considers obsolete and unsupported.
| 0x11 | [`govobj` message](core-ref-p2p-network-governance-messages.md#sectiongovobj)  | 0           | N/A        | Connecting node is using a governance protocol version that the rejecting node considers obsolete and unsupported.
| 0x11 | [`govobjvote` message](core-ref-p2p-network-governance-messages.md#sectiongovobjvote) | 0           | N/A        | Connecting node is using a governance protocol version that the rejecting node considers obsolete and unsupported.
...
| 0x12 | [`tx` message](core-ref-p2p-network-data-messages.md#sectiontx)      | 32          | char[32]   | Duplicate input spend (double spend): the rejected transaction spends the same input as a previously-received transaction.  Extra data may include the rejected transaction's TXID.
| 0x12 | [`version` message](core-ref-p2p-network-control-messages.md#sectionversion) | 0           | N/A        | More than one [`version` message](core-ref-p2p-network-control-messages.md#sectionversion) received in this connection.
| 0x40 | [`tx` message](core-ref-p2p-network-data-messages.md#sectiontx)      | 32          | char[32]   | The transaction will not be mined or relayed because the rejecting node considers it non-standard---a transaction type or version unknown by the server.  Extra data may include the rejected transaction's TXID.
| 0x41 | [`tx` message](core-ref-p2p-network-data-messages.md#sectiontx)      | 32          | char[32]   | One or more output amounts are below the dust threshold.  Extra data may include the rejected transaction's TXID.
| 0x42 | [`tx` message](core-ref-p2p-network-data-messages.md#sectiontx)      |             | char[32]   | The transaction did not have a large enough fee or priority to be relayed or mined.  Extra data may include the rejected transaction's TXID.
| 0x43 | [`block` message](core-ref-p2p-network-data-messages.md#sectionblock)   | 32          | char[32]   | The block belongs to a block chain which is not the same block chain as provided by a compiled-in checkpoint.  Extra data may include the rejected block's header hash.
...
The annotated hexdump below shows a [`reject` message](core-ref-p2p-network-control-messages.md#sectionreject). (The message header has been omitted.)
...
The [`sendcmpct` message](core-ref-p2p-network-control-messages.md#sectionsendcmpct) tells the receiving <<glossary:peer>> whether or not to announce new <<glossary:blocks>> using a [`cmpctblock` message](core-ref-p2p-network-data-messages.md#sectioncmpctblock). It also sends the compact block protocol version it supports. The [`sendcmpct` message](core-ref-p2p-network-control-messages.md#sectionsendcmpct) is defined as a message containing a 1-byte integer followed by a 8-byte integer. The first integer is interpreted as a boolean and should have a value of either 1 or 0. The second integer is be interpreted as a little-endian version number.

Upon receipt of a [`sendcmpct` message](core-ref-p2p-network-control-messages.md#sectionsendcmpct) with the first and second integers set to 1, the <<glossary:node>> should announce new blocks by sending a [`cmpctblock` message](core-ref-p2p-network-data-messages.md#sectioncmpctblock).

Upon receipt of a [`sendcmpct` message](core-ref-p2p-network-control-messages.md#sectionsendcmpct) with the first integer set to 0, the node shouldn't announce new blocks by sending a [`cmpctblock` message](core-ref-p2p-network-data-messages.md#sectioncmpctblock), but instead announce new blocks by sending invs or <<glossary:headers>>, as defined by [BIP130](https://github.com/bitcoin/bips/blob/master/bip-0130.mediawiki).

Upon receipt of a [`sendcmpct` message](core-ref-p2p-network-control-messages.md#sectionsendcmpct) with the second integer set to something other than 1, nodes should treat the peer as if they had not received the message (as it indicates the peer will provide an unexpected encoding in [`cmpctblock` messages](core-ref-p2p-network-data-messages.md#sectioncmpctblock), and/or other, messages). This allows future versions to send duplicate [`sendcmpct` messages](core-ref-p2p-network-control-messages.md#sectionsendcmpct) with different versions as a part of a version handshake.

Nodes should check for a protocol version of >= 70209 before sending [`sendcmpct` messages](core-ref-p2p-network-control-messages.md#sectionsendcmpct). Nodes shouldn't send a request for a `MSG_CMPCT_BLOCK` object to a peer before having received a [`sendcmpct` message](core-ref-p2p-network-control-messages.md#sectionsendcmpct) from that peer. Nodes shouldn't request a `MSG_CMPCT_BLOCK` object before having sent all [`sendcmpct` messages](core-ref-p2p-network-control-messages.md#sectionsendcmpct) to that peer which they intend to send, as the peer cannot know what protocol version to use in the response.

The structure of a [`sendcmpct` message](core-ref-p2p-network-control-messages.md#sectionsendcmpct) is defined below.

| Bytes    | Name          | Data Type        | Description
|----------|---------------|------------------|--------------
| 1        | announce      | bool             | 0 - Announce blocks via [`headers` message](core-ref-p2p-network-data-messages.md#sectionheaders) or [`inv` message](core-ref-p2p-network-data-messages.md#sectioninv)<br>1 - Announce blocks via [`cmpctblock` message](core-ref-p2p-network-data-messages.md#sectioncmpctblock)
| 8        | version       | uint64_t         | The compact block protocol version number

The annotated hexdump below shows a [`sendcmpct` message](core-ref-p2p-network-control-messages.md#sectionsendcmpct). (The message header has been omitted.)
...
The [`senddsq` message](core-ref-p2p-network-control-messages.md#sectionsenddsq) is used to notify a <<glossary:peer>> whether or not to send [`dsq` messages](core-ref-p2p-network-privatesend-messages.md#sectiondsq). This allows clients that are not interested in PrivateSend mixing (e.g. mobile <<glossary:wallet>>) to minimize data usage.

| Bytes | Name | Data type | Description |
| --- | --- | --- | --- |
| 1 | fSendDSQueue | bool | 0 - Notify peer to not send any [`dsq` messages](core-ref-p2p-network-privatesend-messages.md#sectiondsq)<br>1 - Notify peer to send all [`dsq` messages](core-ref-p2p-network-privatesend-messages.md#sectiondsq)

The following annotated hexdump shows a [`senddsq` message](core-ref-p2p-network-control-messages.md#sectionsenddsq). (The message header has been omitted.)
...
The [`sendheaders` message](core-ref-p2p-network-control-messages.md#sectionsendheaders) tells the receiving <<glossary:peer>> to send new <<glossary:block>> announcements using a [`headers` message](core-ref-p2p-network-data-messages.md#sectionheaders) rather than an [`inv` message](core-ref-p2p-network-data-messages.md#sectioninv).

There is no payload in a [`sendheaders` message](core-ref-p2p-network-control-messages.md#sectionsendheaders).  See the [message header section](core-ref-p2p-network-message-headers) for an example of a message without a payload.
...
A [`spork` message](core-ref-p2p-network-control-messages.md#sectionspork) may be sent in response to a [`getsporks` message](core-ref-p2p-network-control-messages.md#sectiongetsporks).

The [`spork` message](core-ref-p2p-network-control-messages.md#sectionspork) tells the receiving peer the status of the spork defined by the SporkID field. Upon receiving a <<glossary:spork>> message, the client must verify the <<glossary:signature>> before accepting the spork message as valid.
...
[/block]
To verify `vchSig`, compare the hard-coded spork public key (`strSporkPubKey` from [`src/chainparams.cpp`](https://github.com/dashpay/dash/blob/eaf90b77177efbaf9cbed46e822f0d794f1a0ee5/src/chainparams.cpp#L158)) with the public key recovered from the [`spork` message](core-ref-p2p-network-control-messages.md#sectionspork)'s hash and `vchSig` value (implementation details for Dash Core can be found in `CPubKey::RecoverCompact`). The hash is a double SHA-256 hash of:
...
The following annotated hexdump shows a [`spork` message](core-ref-p2p-network-control-messages.md#sectionspork).
...
The [`verack` message](core-ref-p2p-network-control-messages.md#sectionverack) acknowledges a previously-received [`version` message](core-ref-p2p-network-control-messages.md#sectionversion), informing the connecting <<glossary:node>> that it can begin to send other messages. The [`verack` message](core-ref-p2p-network-control-messages.md#sectionverack) has no payload; for an example of a message with no payload, see the [message headers section](core-ref-p2p-network-message-headers).

# version

The [`version` message](core-ref-p2p-network-control-messages.md#sectionversion) provides information about the transmitting <<glossary:node>> to the receiving node at the beginning of a connection. Until both <<glossary:peers>> have exchanged [`version` messages](core-ref-p2p-network-control-messages.md#sectionversion), no other messages will be accepted.

If a [`version` message](core-ref-p2p-network-control-messages.md#sectionversion) is accepted, the receiving node should send a [`verack` message](core-ref-p2p-network-control-messages.md#sectionverack)---but no node should send a [`verack` message](core-ref-p2p-network-control-messages.md#sectionverack) before initializing its half of the connection by first sending a [`version` message](core-ref-p2p-network-control-messages.md#sectionversion).

Protocol version 70214 added a <<glossary:masternode>> authentication (challenge/response) system. Following the [`verack` message](core-ref-p2p-network-control-messages.md#sectionverack), masternodes should send a [`mnauth` message](core-ref-p2p-network-masternode-messages.md#sectionmnauth) that signs the `mnauth_challenge` with their BLS operator key.
...
| 1        | relay                 | bool             | Optional                                 | *Added in protocol version 70001 as described by BIP37.* <br><br>Transaction relay flag.  If 0x00, no [`inv` messages](core-ref-p2p-network-data-messages.md#sectioninv) or [`tx` messages](core-ref-p2p-network-data-messages.md#sectiontx) announcing new transactions should be sent to this client until it sends a [`filterload` message](core-ref-p2p-network-control-messages.md#sectionfilterload) or [`filterclear` message](core-ref-p2p-network-control-messages.md#sectionfilterclear).  If the relay field is not present or is set to 0x01, this node wants [`inv` messages](core-ref-p2p-network-data-messages.md#sectioninv) and [`tx` messages](core-ref-p2p-network-data-messages.md#sectiontx) announcing new transactions.
| 32       | mnauth_<br>challenge  | uint256          | Optional                                 | *Added in protocol version 70214* <br><br>A challenge to be signed by the receiving masternode. The response is returned via a [`mnauth` message](core-ref-p2p-network-masternode-messages.md#sectionmnauth) following the [`verack` message](core-ref-p2p-network-control-messages.md#sectionverack).
...
The following annotated hexdump shows a [`version` message](core-ref-p2p-network-control-messages.md#sectionversion). (The message header has been omitted and the actual IP addresses have been replaced with [RFC5737](http://tools.ietf.org/html/rfc5737) reserved IP addresses.)
