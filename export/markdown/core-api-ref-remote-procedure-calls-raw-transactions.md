...
The [`combinerawtransaction` RPC](core-api-ref-remote-procedure-calls-raw-transactions.md#sectioncombine-raw-transaction) combine multiple partially signed transactions into one transaction.
...
* [CreateRawTransaction](/docs/core-api-ref-remote-procedure-calls-raw-transactions.md#sectioncreate-raw-transaction): creates an unsigned serialized transaction that spends a previous output to a new output with a P2PKH or P2SH address. The transaction is not stored in the wallet or transmitted to the network.
* [DecodeRawTransaction](/docs/core-api-ref-remote-procedure-calls-raw-transactions.md#sectiondecode-raw-transaction): decodes a serialized transaction hex string into a JSON object describing the transaction.
* [SignRawTransaction](/docs/core-api-ref-remote-procedure-calls-raw-transactions.md#sectionsign-raw-transaction): signs a transaction in the serialized transaction format using private keys stored in the wallet or provided in the call.
...
The [`createrawtransaction` RPC](core-api-ref-remote-procedure-calls-raw-transactions.md#sectioncreate-raw-transaction) creates an unsigned serialized transaction that spends a previous output to a new output with a P2PKH or P2SH address. The transaction is not stored in the wallet or transmitted to the network.
...
* [CombineRawTransaction](/docs/core-api-ref-remote-procedure-calls-raw-transactions.md#sectioncombine-raw-transaction): combine multiple partially signed transactions into one transaction.
* [DecodeRawTransaction](/docs/core-api-ref-remote-procedure-calls-raw-transactions.md#sectiondecode-raw-transaction): decodes a serialized transaction hex string into a JSON object describing the transaction.
* [SignRawTransaction](/docs/core-api-ref-remote-procedure-calls-raw-transactions.md#sectionsign-raw-transaction): signs a transaction in the serialized transaction format using private keys stored in the wallet or provided in the call.
* [SendRawTransaction](/docs/core-api-ref-remote-procedure-calls-raw-transactions.md#sectionsend-raw-transaction): validates a transaction and broadcasts it to the peer-to-peer network.
* [Serialized Transaction Format](core-ref-transactions-raw-transaction-format)
...
The [`decoderawtransaction` RPC](core-api-ref-remote-procedure-calls-raw-transactions.md#sectiondecode-raw-transaction) decodes a serialized transaction hex string into a JSON object describing the transaction.
...
The [`decodescript` RPC](core-api-ref-remote-procedure-calls-raw-transactions.md#sectiondecode-script) decodes a hex-encoded P2SH redeem script.
...
The [`fundrawtransaction` RPC](core-api-ref-remote-procedure-calls-raw-transactions.md#sectionfund-raw-transaction) adds inputs to a transaction until it has enough in value to meet its out value.  This will not modify existing inputs, and will add one change output to the outputs.
...
The [`getrawtransaction` RPC](core-api-ref-remote-procedure-calls-raw-transactions.md#sectionget-raw-transaction) gets a hex-encoded serialized transaction or a JSON object describing the transaction. By default, Dash Core only stores complete transaction data for UTXOs and your own transactions, so the RPC may fail on historic transactions unless you use the non-default `txindex=1` in your Dash Core startup settings.
...
* [GetSpecialTxes](/docs/core-api-ref-remote-procedure-calls-blockchain.md#sectionget-special-txes): returns an array of special transactions found in the specified block
...
The [`sendrawtransaction` RPC](core-api-ref-remote-procedure-calls-raw-transactions.md#sectionsend-raw-transaction) validates a transaction and broadcasts it to the peer-to-peer network.
...
The [`signrawtransaction` RPC](core-api-ref-remote-procedure-calls-raw-transactions.md#sectionsign-raw-transaction) signs a transaction in the serialized transaction format using private keys stored in the wallet or provided in the call.
...
Name | Type | Presence | Description
--- | --- | --- | ---
SigHash | string | Optional<br>(0 or 1) | The type of signature hash to use for all of the signatures performed.  (You must use separate calls to the [`signrawtransaction` RPC](core-api-ref-remote-procedure-calls-raw-transactions.md#sectionsign-raw-transaction) if you want to use different signature hash types for different signatures.  The allowed values are: `ALL`, `NONE`, `SINGLE`, `ALL|ANYONECANPAY`, `NONE|ANYONECANPAY`, and `SINGLE|ANYONECANPAY`
