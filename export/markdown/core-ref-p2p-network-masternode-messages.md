...
The [`ssc` message](core-ref-p2p-network-masternode-messages.md#sectionssc) is used to track the sync status of masternode objects. This message is sent in response to sync requests for the list of governance objects (`govsync` message), and governance object votes (`govsync` message).
...
| 10 | MASTERNODE_SYNC_GOVOBJ | [`govsync` message](core-ref-p2p-network-governance-messages.md#sectiongovsync)
| 11 | MASTERNODE_SYNC_GOVOBJ_VOTE | [`govsync` message](core-ref-p2p-network-governance-messages.md#sectiongovsync) with non-zero hash

The following annotated hexdump shows a [`ssc` message](core-ref-p2p-network-masternode-messages.md#sectionssc). (The message header has been omitted.)
...
The [`mnauth` message](core-ref-p2p-network-masternode-messages.md#sectionmnauth) is sent by a <<glossary:masternode>> immediately after sending a [`verack` message](core-ref-p2p-network-control-messages.md#sectionverack) to authenticate that the sender is a masternode. It is only sent when the sender is actually a masternode.

The [`mnauth` message](core-ref-p2p-network-masternode-messages.md#sectionmnauth) signs a challenge that was previously sent via a [`version` message](core-ref-p2p-network-control-messages.md#sectionversion). The challenge is signed differently depending on if the connection is inbound or outbound.
...
The following annotated hexdump shows a [`mnauth` message](core-ref-p2p-network-masternode-messages.md#sectionmnauth). (The message header has been omitted.)
