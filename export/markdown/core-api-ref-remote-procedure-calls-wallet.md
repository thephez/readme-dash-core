...
The [`abandontransaction` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionabandon-transaction) marks an in-wallet transaction and all its in-wallet descendants as abandoned. This allows their inputs to be respent.
...
* [SendRawTransaction](/docs/core-api-ref-remote-procedure-calls-raw-transactions.md#sectionsend-raw-transaction): validates a transaction and broadcasts it to the peer-to-peer network.

# AbortRescan

The [`abortrescan` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionabort-rescan) Stops current wallet rescan

Stops current wallet rescan triggered e.g. by an [`importprivkey` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionimport-priv-key) call.
...
The [`addmultisigaddress` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionadd-multi-sig-address) adds a P2SH multisig address to the wallet.
...
* [CreateMultiSig](/docs/core-api-ref-remote-procedure-calls-util.md#sectioncreate-multi-sig): creates a P2SH multi-signature address.
* [DecodeScript](/docs/core-api-ref-remote-procedure-calls-raw-transactions.md#sectiondecode-script): decodes a hex-encoded P2SH redeem script.
...
The [`backupwallet` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionbackup-wallet) safely copies `wallet.dat` to the specified file, which can be a directory or a path with filename.
...
* [DumpWallet](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectiondump-wallet): creates or overwrites a file with all wallet keys in a human-readable format.
* [ImportWallet](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionimport-wallet): imports private keys from a file in wallet dump file format (see the [`dumpwallet` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectiondump-wallet)). These keys will be added to the keys currently in the wallet.  This call may need to rescan all or parts of the block chain for transactions affecting the newly-added keys, which may take several minutes.

# DumpHDInfo

The [`dumphdinfo` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectiondump-hd-info) returns an object containing sensitive private info about this HD wallet
...
The [`dumpprivkey` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectiondump-priv-key) returns the wallet-import-format (WIP) private key corresponding to an address. (But does not remove it from the wallet.)
...
* [ImportPrivKey](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionimport-priv-key): adds a private key to your wallet. The key should be formatted in the wallet import format created by the [`dumpprivkey` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectiondump-priv-key).
* [DumpWallet](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectiondump-wallet): creates or overwrites a file with all wallet keys in a human-readable format.
...
The [`dumpwallet` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectiondump-wallet) creates or overwrites a file with all wallet keys in a human-readable format.
...
* [BackupWallet](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionbackup-wallet): safely copies `wallet.dat` to the specified file, which can be a directory or a path with filename.
* [ImportWallet](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionimport-wallet): imports private keys from a file in wallet dump file format (see the [`dumpwallet` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectiondump-wallet)). These keys will be added to the keys currently in the wallet.  This call may need to rescan all or parts of the block chain for transactions affecting the newly-added keys, which may take several minutes.
...
The [`encryptwallet` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionencrypt-wallet) encrypts the wallet with a passphrase.  This is only to enable encryption for the first time. After encryption is enabled, you will need to enter the passphrase to use private keys.
[block:callout]
{
  "type": "warning",
  "body": "**Warning:** if using this RPC on the command line, remember that your shell probably saves your command lines (including the value of the passphrase parameter). In addition, there is no RPC to completely disable encryption. If you want to return to an unencrypted wallet, you must create a new wallet and restore your data from a backup made with the [`dumpwallet` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectiondump-wallet)."
}
[/block]
...
* [WalletPassphrase](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionwallet-passphrase): stores the wallet decryption key in memory for the indicated number of seconds. Issuing the `walletpassphrase` command while the wallet is already unlocked will set a new unlock time that overrides the old one.
* [WalletLock](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionwallet-lock): removes the wallet encryption key from memory, locking the wallet. After calling this method, you will need to call `walletpassphrase` again before being able to call any methods which require the wallet to be unlocked.
* [WalletPassphraseChange](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionwallet-passphrase-change): changes the wallet passphrase from 'old passphrase' to 'new passphrase'.
...
The [`getbalance` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionget-balance) gets the balance in decimal dash across all accounts or for a particular account.
...
* [ListAccounts](/docs/core-api-ref-remote-procedure-calls-wallet-deprecated.md#sectionlist-accounts): lists accounts and their balances.
* [GetReceivedByAccount](/docs/core-api-ref-remote-procedure-calls-wallet-deprecated.md#sectionget-received-by-account): returns the total amount received by addresses in a particular account from transactions with the specified number of confirmations.  It does not count coinbase transactions.
* [GetReceivedByAddress](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionget-received-by-address): returns the total amount received by the specified address in transactions with the specified number of confirmations. It does not count coinbase transactions.
...
The [`getnewaddress` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionget-new-address) returns a new Dash address for receiving payments. If an account is specified, payments received with the address will be credited to that account.
...
* [GetAccountAddress](/docs/core-api-ref-remote-procedure-calls-wallet-deprecated.md#sectionget-account-address): returns the current Dash address for receiving payments to this account. If the account doesn't exist, it creates both the account and a new address for receiving payment.  Once a payment has been received to an address, future calls to this RPC for the same account will return a different address.
* [GetRawChangeAddress](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionget-raw-change-address): returns a new Dash address for receiving change. This is for use with raw transactions, not normal use.
...
The [`getrawchangeaddress` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionget-raw-change-address) returns a new Dash address for receiving change. This is for use with raw transactions, not normal use.
...
* [GetNewAddress](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionget-new-address): returns a new Dash address for receiving payments. If an account is specified, payments received with the address will be credited to that account.
* [GetAccountAddress](/docs/core-api-ref-remote-procedure-calls-wallet-deprecated.md#sectionget-account-address): returns the current Dash address for receiving payments to this account. If the account doesn't exist, it creates both the account and a new address for receiving payment.  Once a payment has been received to an address, future calls to this RPC for the same account will return a different address.
...
The [`getreceivedbyaddress` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionget-received-by-address) returns the total amount received by the specified address in transactions with the specified number of confirmations. It does not count coinbase transactions.
...
* [GetReceivedByAccount](/docs/core-api-ref-remote-procedure-calls-wallet-deprecated.md#sectionget-received-by-account): returns the total amount received by addresses in a particular account from transactions with the specified number of confirmations.  It does not count coinbase transactions.
* [GetAddressesByAccount](/docs/core-api-ref-remote-procedure-calls-wallet-deprecated.md#sectionget-addresses-by-account): returns a list of every address assigned to a particular account.
* [ListAccounts](/docs/core-api-ref-remote-procedure-calls-wallet-deprecated.md#sectionlist-accounts): lists accounts and their balances.
...
The [`gettransaction` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionget-transaction) gets detailed information about an in-wallet transaction.
...
→ <br>`instantlock` | bool | Required<br>(exactly 1) | **Always `false` if [lite mode](core-guide-dash-features.md#sectionlite-mode) is enabled**<br><br>Current transaction lock state (InstantSend and/or ChainLock)
→ <br>`instantlock_internal` | bool | Required<br>(exactly 1) | **Always `false` if [lite mode](core-guide-dash-features.md#sectionlite-mode) is enabled**<br><br>Current InstantSend transaction lock state
→ <br>`chainlock` | bool | Required<br>(exactly 1) | *Added in Dash Core 0.14.0*<br><br>**Always `false` if [lite mode](core-guide-dash-features.md#sectionlite-mode) is enabled**<br><br>If set to `true`, this transaction is in a block that is locked (not susceptible to a chain re-org)  
...
* [GetRawTransaction](/docs/core-api-ref-remote-procedure-calls-raw-transactions.md#sectionget-raw-transaction): gets a hex-encoded serialized transaction or a JSON object describing the transaction. By default, Dash Core only stores complete transaction data for UTXOs and your own transactions, so the RPC may fail on historic transactions unless you use the non-default `txindex=1` in your Dash Core startup settings.
...
The [`getunconfirmedbalance` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionget-unconfirmed-balance) returns the wallet's total unconfirmed balance.
...
The [`getwalletinfo` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionget-wallet-info) provides information about the wallet.
...
→<br>`balance` | number (dash) | Required<br>(exactly 1) | The total confirmed balance of the wallet.  The same as returned by the [`getbalance` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionget-balance) with default parameters
...
→<br>`unconfirmed_balance` | number (dash) | Required<br>(exactly 1) | The total unconfirmed balance of the wallet.  The same as returned by the [`getunconfirmedbalance` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionget-unconfirmed-balance) with default parameters
...
→<br>`unlocked_until` | number (int) | Optional<br>(0 or 1) | Only returned if the wallet was encrypted with the [`encryptwallet` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionencrypt-wallet). A Unix epoch date when the wallet will be locked, or `0` if the wallet is currently locked
...
* [ListTransactions](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionlist-transactions): returns the most recent transactions that affect the wallet.
...
The [`importaddress` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionimport-address) adds an address or pubkey script to the wallet without the associated private key, allowing you to watch for transactions affecting that address or pubkey script without being able to spend any of its outputs.
...
* [ImportPrivKey](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionimport-priv-key): adds a private key to your wallet. The key should be formatted in the wallet import format created by the [`dumpprivkey` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectiondump-priv-key).
* [ListReceivedByAddress](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionlist-received-by-address): lists the total number of dash received by each address.

# ImportElectrumWallet

The [`importelectrumwallet` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionimport-electrum-wallet) imports keys from an Electrum wallet export file (.csv or .json)
...
The [`importmulti` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionimport-multi) imports addresses or scripts (with private keys, public keys, or P2SH redeem scripts) and optionally performs the minimum necessary rescan for all imports.
...
* [ImportPrivKey](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionimport-priv-key): adds a private key to your wallet. The key should be formatted in the wallet import format created by the [`dumpprivkey` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectiondump-priv-key).
* [ImportAddress](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionimport-address): adds an address or pubkey script to the wallet without the associated private key, allowing you to watch for transactions affecting that address or pubkey script without being able to spend any of its outputs.
* [ImportWallet](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionimport-wallet): imports private keys from a file in wallet dump file format (see the [`dumpwallet` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectiondump-wallet)). These keys will be added to the keys currently in the wallet.  This call may need to rescan all or parts of the block chain for transactions affecting the newly-added keys, which may take several minutes.
...
The [`importprivkey` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionimport-priv-key) adds a private key to your wallet. The key should be formatted in the wallet import format created by the [`dumpprivkey` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectiondump-priv-key).
...
* [DumpPrivKey](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectiondump-priv-key): returns the wallet-import-format (WIP) private key corresponding to an address. (But does not remove it from the wallet.)
* [ImportAddress](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionimport-address): adds an address or pubkey script to the wallet without the associated private key, allowing you to watch for transactions affecting that address or pubkey script without being able to spend any of its outputs.
* [ImportPubKey](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionimport-pub-key): imports a public key (in hex) that can be watched as if it were in your wallet but cannot be used to spend
* [ImportWallet](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionimport-wallet): imports private keys from a file in wallet dump file format (see the [`dumpwallet` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectiondump-wallet)). These keys will be added to the keys currently in the wallet.  This call may need to rescan all or parts of the block chain for transactions affecting the newly-added keys, which may take several minutes.
...
The [`importprunedfunds` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionimport-pruned-funds) imports funds without the need of a rescan. Meant for use with pruned wallets. Corresponding address or script must previously be included in wallet.
...
* [ImportPrivKey](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionimport-priv-key): adds a private key to your wallet. The key should be formatted in the wallet import format created by the [`dumpprivkey` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectiondump-priv-key).
* [RemovePrunedFunds](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionremove-pruned-funds): deletes the specified transaction from the wallet. Meant for use with pruned wallets and as a companion to importprunedfunds.

# ImportPubKey

The [`importpubkey` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionimport-pub-key) imports a public key (in hex) that can be watched as if it were in your wallet but cannot be used to spend
...
The [`importwallet` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionimport-wallet) imports private keys from a file in wallet dump file format (see the [`dumpwallet` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectiondump-wallet)). These keys will be added to the keys currently in the wallet.  This call may need to rescan all or parts of the block chain for transactions affecting the newly-added keys, which may take several minutes.
...
Import the file shown in the example subsection of the [`dumpwallet` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectiondump-wallet).
...
The [`keepass` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionkee-pass) provides commands for configuring and managing KeePass authentication
...
The [`keypoolrefill` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionkey-pool-refill) fills the cache of unused pre-generated keys (the keypool).
...
The [`listaddressbalances` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionlist-address-balances) lists addresses of this wallet and their balances
...
* [ListAddressGroupings](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionlist-address-groupings): lists groups of addresses that may have had their common ownership made public by common use as inputs in the same transaction or from being used as change from a previous transaction.
...
The [`listaddressgroupings` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionlist-address-groupings) lists groups of addresses that may have had their common ownership made public by common use as inputs in the same transaction or from being used as change from a previous transaction.
...
* [GetAddressesByAccount](/docs/core-api-ref-remote-procedure-calls-wallet-deprecated.md#sectionget-addresses-by-account): returns a list of every address assigned to a particular account.
* [GetTransaction](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionget-transaction): gets detailed information about an in-wallet transaction.
* [ListAddressBalances](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionlist-address-balances): lists addresses of this wallet and their balances
...
The [`listlockunspent` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionlist-lock-unspent) returns a list of temporarily unspendable (locked) outputs.
...
* [LockUnspent](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionlock-unspent): temporarily locks or unlocks specified transaction outputs. A locked transaction output will not be chosen by automatic coin selection when spending dash. Locks are stored in memory only, so nodes start with zero locked outputs and the locked output list is always cleared when a node stops or fails.
...
The [`listreceivedbyaddress` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionlist-received-by-address) lists the total number of dash received by each address.
...
* [ListReceivedByAccount](/docs/core-api-ref-remote-procedure-calls-wallet-deprecated.md#sectionlist-received-by-account): lists the total number of dash received by each account.
* [GetReceivedByAddress](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionget-received-by-address): returns the total amount received by the specified address in transactions with the specified number of confirmations. It does not count coinbase transactions.
* [GetReceivedByAccount](/docs/core-api-ref-remote-procedure-calls-wallet-deprecated.md#sectionget-received-by-account): returns the total amount received by addresses in a particular account from transactions with the specified number of confirmations.  It does not count coinbase transactions.
...
The [`listsinceblock` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionlist-since-block) gets all transactions affecting the wallet which have occurred since a particular block, plus the header hash of a block at a particular depth.
...
The [`listtransactions` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionlist-transactions) returns the most recent transactions that affect the wallet.
...
* [GetTransaction](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionget-transaction): gets detailed information about an in-wallet transaction.
* [ListSinceBlock](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionlist-since-block): gets all transactions affecting the wallet which have occurred since a particular block, plus the header hash of a block at a particular depth.
...
The [`listunspent` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionlist-unspent) returns an array of unspent transaction outputs belonging to this wallet. **Note:** as of Bitcoin
...
The [`listwallets` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionlist-wallets) returns a list of currently loaded wallets.

For full information on the wallet, use the [`getwalletinfo` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionget-wallet-info).
...
[/block]
The [`loadwallet` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionload-wallet) loads a wallet from a wallet file or directory. Note that all wallet command-line options used when starting dashd will be applied to the new wallet (eg -zapwallettxes, upgradewallet, rescan, etc).
...
The [`lockunspent` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionlock-unspent) temporarily locks or unlocks specified transaction outputs. A locked transaction output will not be chosen by automatic coin selection when spending dash. Locks are stored in memory only, so nodes start with zero locked outputs and the locked output list is always cleared when a node stops or fails.
...
* [ListLockUnspent](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionlist-lock-unspent): returns a list of temporarily unspendable (locked) outputs.
* [ListUnspent](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionlist-unspent): returns an array of unspent transaction outputs belonging to this wallet.
...
The [`removeprunedfunds` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionremove-pruned-funds) deletes the specified transaction from the wallet. Meant for use with pruned wallets and as a companion to importprunedfunds. This will effect wallet balances.
...
* [ImportPrivKey](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionimport-priv-key): adds a private key to your wallet. The key should be formatted in the wallet import format created by the [`dumpprivkey` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectiondump-priv-key).
* [ImportPrunedFunds](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionimport-pruned-funds): imports funds without the need of a rescan. Meant for use with pruned wallets.


# RescanBlockChain

The [`rescanblockchain` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionrescan-block-chain) rescans the local blockchain for wallet related transactions.
...
* [AbortRescan](core-api-ref-remote-procedure-calls-wallet.md#sectionabort-rescan): stops current wallet rescan. 
...
The [`sendmany` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionsend-many) creates and broadcasts a transaction which sends outputs to multiple addresses.
...
Name | Type | Presence | Description
--- | --- | --- | ---
Confirmations | number (int) | Optional<br>(0 or 1) | The minimum number of confirmations an incoming transaction must have for its outputs to be credited to this account's balance. Outgoing transactions are always counted, as are move transactions made with the [`move` RPC](core-api-ref-remote-procedure-calls-wallet-deprecated.md#sectionmove). If an account doesn't have a balance high enough to pay for this transaction, the payment will be rejected.  Use `0` to spend unconfirmed incoming payments. Default is `1`

[block:callout]
{
  "type": "warning",
  "body": "**Warning:** if account1 receives an unconfirmed payment and transfers it to account2 with the [`move` RPC](core-api-ref-remote-procedure-calls-wallet-deprecated.md#sectionmove), account2 will be able to spend those Dash even if this parameter is set to `1` or higher."
}
[/block]
...
* [SendFrom](/docs/core-api-ref-remote-procedure-calls-wallet-deprecated.md#sectionsend-from): spends an amount from a local account to a dash address.
* [SendToAddress](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionsend-to-address): spends an amount to a given address.
* [Move](/docs/core-api-ref-remote-procedure-calls-wallet-deprecated.md#sectionmove): moves a specified amount from one account in your wallet to another using an off-block-chain transaction.
...
The [`sendtoaddress` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionsend-to-address) spends an amount to a given address.
...
* [SendFrom](/docs/core-api-ref-remote-procedure-calls-wallet-deprecated.md#sectionsend-from): spends an amount from a local account to a dash address.
* [SendMany](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionsend-many): creates and broadcasts a transaction which sends outputs to multiple addresses.
* [Move](/docs/core-api-ref-remote-procedure-calls-wallet-deprecated.md#sectionmove): moves a specified amount from one account in your wallet to another using an off-block-chain transaction.

# SetPrivateSendAmount

The [`setprivatesendamount` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionset-private-send-amount) sets the amount of DASH to be mixed with PrivateSend
...
* [SetPrivateSendRounds](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionset-private-send-rounds): sets the number of PrivateSend mixing rounds to use

# SetPrivateSendRounds

The [`setprivatesendrounds` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionset-private-send-rounds) sets the number of PrivateSend mixing rounds to use
...
* [SetPrivateSendAmount](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionset-private-send-amount): sets the amount of DASH to be mixed with PrivateSend
...
The [`settxfee` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionset-tx-fee) sets the transaction fee per kilobyte paid by transactions created by this wallet.
...
The [`signmessage` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionsign-message) signs a message with the private key of an address.
...
* [VerifyMessage](/docs/core-api-ref-remote-procedure-calls-util.md#sectionverify-message): verifies a signed message.
...
The [`walletlock` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionwallet-lock) removes the wallet encryption key from memory, locking the wallet. After calling this method, you will need to call `walletpassphrase` again before being able to call any methods which require the wallet to be unlocked.
...
* [EncryptWallet](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionencrypt-wallet): encrypts the wallet with a passphrase.  This is only to enable encryption for the first time. After encryption is enabled, you will need to enter the passphrase to use private keys.
* [WalletPassphrase](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionwallet-passphrase): stores the wallet decryption key in memory for the indicated number of seconds. Issuing the `walletpassphrase` command while the wallet is already unlocked will set a new unlock time that overrides the old one.
* [WalletPassphraseChange](/docs/core-api-ref-remote-procedure-calls-wallet.md#sectionwallet-passphrase-change): changes the wallet passphrase from 'old passphrase' to 'new passphrase'.
...
The [`walletpassphrase` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionwallet-passphrase) stores the wallet decryption key in memory for the indicated number of seconds. Issuing the `walletpassphrase` command while the wallet is already unlocked will set a new unlock time that overrides the old one.
...
The [`walletpassphrasechange` RPC](core-api-ref-remote-procedure-calls-wallet.md#sectionwallet-passphrase-change) changes the wallet passphrase from 'old passphrase' to 'new passphrase'.
