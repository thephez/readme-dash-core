...
[/block]
The `ix` message (transaction lock request) has the same structure as the [`tx` message](core-ref-p2p-network-data-messages.md#sectiontx). The masternode network responds with `txlvote` messages if the transaction inputs can be locked.
