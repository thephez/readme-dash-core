...
→ → →<br>`address` | string | Required<br>(exactly 1) | An IP address or .onion address this node believes it listens on.  This may be manually configured, auto detected, or based on [`version` message](core-ref-p2p-network-control-messages#section-version)s this node received from its peers
→ → →<br>`port` | number (int) | Required<br>(exactly 1) | The port number this node believes it listens on for the associated `address`.  This may be manually configured, auto detected, or based on [`version` message](core-ref-p2p-network-control-messages#section-version)s this node received from its peers
...
→ →<br>`synced_blocks` | number (int) | Required<br>(exactly 1) | The highest-height block we have in common with this node based on P2P [`inv` message](core-ref-p2p-network-data-messages#section-inv)s this node sent us.  If no block [`inv` message](core-ref-p2p-network-data-messages#section-inv)s have been received from this node, this will be set to `-1`
